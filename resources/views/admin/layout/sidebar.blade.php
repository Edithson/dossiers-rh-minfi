<aside id="sidebar" class="bg-slate-900 text-white w-64 flex-shrink-0 hidden md:flex flex-col transition-transform duration-300 absolute md:relative z-20 h-full">
    ...
    <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
        <a href="{{ route('dashboard') }}"
            data-match="exact"
            class="nav-link flex items-center gap-3 text-gray-400 hover:bg-slate-800 hover:text-white px-4 py-3 rounded-lg transition-colors">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('.nav-link').forEach(link => {
            ...
            const matchType = link.dataset.match;

            const isActive = matchType === 'exact'
                ? currentPath === linkPath
                : currentPath === linkPath || currentPath.startsWith(linkPath + '/');

            if (isActive) {
                link.classList.remove('text-gray-400', 'hover:bg-slate-800', 'hover:text-white');
